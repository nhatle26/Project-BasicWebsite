...
function renderProducts(productsToRender, theme) {
  const productListId =
    theme === "all" ? "productList" : `productList-${theme}`;
  ...
  if (productsToRender.length === 0) {
    productListElement.innerHTML =
      '<p style="grid-column: 1/-1; text-align: center; padding: 40px; font-size: 18px; color: #999;">Không tìm thấy sản phẩm nào</p>';
    return;
  }

  console.log(
    `🎨 Render ${productsToRender.length} sản phẩm cho theme: ${theme}`
  );
  ...
  productsToRender.forEach((product) => {
    ...
    productCard.innerHTML = `
      ...
      product.stock <= 0 ? "disabled" : ""
            ...
            <a href="#" onclick="viewProductDetail('${
              product.id
            }'); return false;" style="display: inline-block; margin-top: 10px; color: #2196F3; text-decoration: none;">
...
function filterProductsByTheme(theme) {
  ...
  if (theme !== "all") {
    ...
    filteredProducts = filteredProducts.filter(
      (product) =>
        product.event &&
        Array.isArray(product.event) &&
        product.event.includes(theme)
    );
  }
...
function filterProducts(theme = "all") {
  let categoryFilter = document.getElementById(
...
function search() {
  ...
  const theme =
    document.querySelector(".theme-tab.active")?.getAttribute("data-theme") ||
    "all";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (currentUser) {
    ...
    if (currentUser.role === "admin") {
      navHTML += `
                <a href="admin.html" id="userDisplayName" style ="color: #f8f8f8ff;">
                    <i class="fas fa-user-shield"></i> Quản trị
                </a>
            `;
    } else {
      navHTML += `
                <a href="#" id="userDisplayName" style ="color: #f8f8f8ff;">
    ...
    navHTML += `
            <a href="#" onclick="logout(); return false;" style="color: #f8f8f8ff;">
